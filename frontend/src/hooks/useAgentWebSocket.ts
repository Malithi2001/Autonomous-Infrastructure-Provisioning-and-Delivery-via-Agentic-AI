...
function parseWebSocketEvent(raw: string): AgentWebSocketEvent | null {
  try {
    ...
    if (parsed && typeof parsed === 'object' && 'event' in parsed) return parsed as AgentWebSocketEvent
    return null
  } catch {
    return null
  }
...
export function useAgentWebSocket({ sessionId, onToken, onDone, onError, onPing }: UseAgentWebSocketOptions) {
  ...
  const connect = useCallback(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (parsed.event === 'done') callbacksRef.current.onDone?.(parsed.session_id)
      if (parsed.event === 'error') {
        const message = parsed.detail || parsed.message || 'WebSocket chat failed.'
  ...
  const sendMessage = useCallback(({ message, sessionId: overrideSessionId }: SendMessageOptions) => {
    const socket = wsRef.current
    if (!socket || socket.readyState !== WebSocket.OPEN) return false
    socket.send(JSON.stringify({ message, session_id: overrideSessionId || sessionId || undefined }))
    return true
  }, [sessionId])
