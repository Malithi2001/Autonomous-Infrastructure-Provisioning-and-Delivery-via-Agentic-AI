...
export default function UsersPage() {
  ...
  const usersByRole = useMemo(() => {
    return ROLE_ORDER.reduce<Record<UserRole, number>>(
      (acc, role) => {
  ...
  const handleCreateUser = async (event: FormEvent) => {
    ...
    try {
      ...
      setSuccess(
        `${ROLE_DEFINITIONS[form.role].label} user ${created.username} was created.`,
      );
      setForm(initialForm);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-ink-subtle">
                Admin-only provisioning for safe role-based AI DevOps access
              </p>
            </div>
          </div>
            ...
            {ROLE_ORDER.map((role) => (
              ...
              >
                ...
                <p className="mt-0.5 opacity-80">
                  {usersByRole[role]} account
                  {usersByRole[role] === 1 ? "" : "s"}
                </p>
              </div>
            ))}
          </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-5 overflow-y-auto px-4 py-5 md:px-6 md:py-6 xl:grid-cols-[420px_1fr]">
        <form onSubmit={handleCreateUser} className="card h-fit p-5" noValidate>
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-ink-subtle">
                Use this for Admin and Operator accounts. Public signup remains
                limited to Developer and Viewer.
              </p>
            </div>
          </div>

          {error && (
            <div className="mb-4 rounded-2xl border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-700 dark:text-red-200">
              {error}
            </div>
          )}
          {success && (
            <div className="mb-4 flex items-center gap-2 rounded-2xl border border-primary-500/30 bg-primary-500/10 px-4 py-3 text-sm text-primary-700 dark:text-primary-200">
              <CheckCircle size={16} />
              {success}
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-xs font-medium uppercase tracking-[0.16em] text-ink-subtle">
                Role
              </label>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 leading-4 opacity-80">
                        {ROLE_DEFINITIONS[role].headline}
                      </p>
                    </button>
                  );
                })}
            ...
            <div>
              ...
              <input
                ...
                type="email"
                placeholder="operator@devops.example.com"
          ...
          <button
            type="submit"
            disabled={submitting}
            className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-primary-600 px-4 py-3 text-sm font-semibold text-white shadow-glow transition hover:bg-primary-500 disabled:cursor-not-allowed disabled:bg-surface-600 disabled:text-ink-faint disabled:shadow-none"
          >
            {submitting ? (
          ...
          {loading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-center">
              <div className="mb-4 rounded-3xl border border-surface-600 bg-surface-800 p-4">
                <UsersRound size={40} className="text-ink-subtle" />
              </div>
              <p className="font-medium text-ink">No users found</p>
              <p className="mt-1 text-sm text-ink-subtle">
                Create your first RBAC user from the form.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {users.map((user) => {
                ...
                return (
                  <div
                    key={user.id}
                    className="card flex flex-col gap-4 px-4 py-4 transition-colors hover:border-primary-500/30 sm:flex-row sm:items-center"
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                      ...
                      <span className="rounded-full border border-surface-600 px-2.5 py-1 text-[11px] text-ink-subtle">
