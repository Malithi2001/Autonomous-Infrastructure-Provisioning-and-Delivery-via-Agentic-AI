...
import { CheckCircle, XCircle, Clock, AlertTriangle, ShieldCheck, Loader2 } from 'lucide-react'
...
export default function ApprovalsPage() {
  ...
  const fetchApprovals = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.message || 'Unable to load approval requests.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const decide = async (id: string, approved: boolean) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.message || 'Unable to submit this approval decision.')
    } finally {
      setDecidingId(null)
    }
  }
    ...
    <div className="flex h-full flex-col bg-surface-900">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {loading ? <div className="flex h-32 items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-primary-500 border-t-transparent" /></div>
          : error ? <div className="mx-auto max-w-3xl rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-700 dark:text-red-200">{error}</div>
          : approvals.length === 0 ? <div className="flex h-full flex-col items-center justify-center text-center"><div className="mb-4 rounded-3xl border border-primary-500/25 bg-primary-500/10 p-4"><CheckCircle size={40} className="text-primary-600 dark:text-primary-300" /></div><p className="font-medium text-ink">No pending approvals</p><p className="mt-1 text-sm text-ink-subtle">High-risk agent actions will appear here for your review.</p></div>
            : <div className="mx-auto max-w-3xl space-y-4">{approvals.map((a) => {
              const description = a.description || a.action || a.summary || 'Approval requested by the agent.'
              const details = parseApprovalDetails(a)
              const risk = details?.risk_level || a.risk_level
              return <div key={a.id} className="card p-5 transition hover:border-primary-500/30">
                <div className="mb-4 flex items-start gap-3">
                  <AlertTriangle size={18} className="mt-0.5 shrink-0 text-amber-500" />
                  <div className="flex-1">
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      <p className="text-sm font-medium text-ink">{description}</p>
                      {risk && <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium uppercase text-amber-700 dark:text-amber-300">{risk}</span>}
                    </div>
                    <div className="flex flex-wrap items-center gap-3 text-xs text-ink-subtle">
                      <span className="flex items-center gap-1"><Clock size={11} />{relativeTime(a.created_at)}</span>
                      {a.requested_by && <span>Requested by {a.requested_by}</span>}
                      {a.expires_at && <span className="text-amber-700 dark:text-amber-300">Expires {relativeTime(a.expires_at)}</span>}
                    </div>
                  </div>
                </div>

                {details && <div className="mb-4 rounded-lg border border-surface-600 bg-surface-800/60 p-4">
                  <dl className="grid gap-4 sm:grid-cols-2">
                    <DetailRow label="Repository" value={details.repository} />
                    <DetailRow label="Workflow run" value={details.workflow_run_id} />
                    <DetailRow label="Predicted failure" value={details.predicted_failure} />
                    <DetailRow label="Workflow file" value={details.workflow_path} />
                    <DetailRow label="Suggested fix" value={details.suggested_fix} />
                    <DetailRow label="Workflow URL" value={details.workflow_url} />
                  </dl>
                  {details.proposed_file_changes && details.proposed_file_changes.length > 0 && <div className="mt-4">
                    <p className="text-[11px] font-medium uppercase tracking-wide text-ink-faint">Proposed file changes</p>
                    <ul className="mt-2 space-y-1 text-sm text-ink-subtle">
                      {details.proposed_file_changes.map((change, index) => <li key={`${a.id}-${index}`}>{change}</li>)}
                    </ul>
                  </div>}
                </div>}

                {a.status === 'pending' && <div className="flex flex-wrap gap-3">
                  <button
                  ...
                  >
                    {decidingId === a.id ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                    Approve
                  </button>
                  <button
                  ...
                  >
                    {decidingId === a.id ? <Loader2 size={14} className="animate-spin" /> : <XCircle size={14} />}
                    Reject
                  </button>
                </div>}
              </div>
