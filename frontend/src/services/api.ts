...
export const executionService = {
  list: async (params: { limit?: number; tool?: string | null; status?: string | null; actor?: string | null; source?: string | null; days?: number } = {}) => {
    const cleaned = Object.fromEntries(Object.entries({ limit: 50, ...params }).filter(([, value]) => value !== null && value !== undefined && value !== ''))
...
export const workflowFailureService = {
  list: async (limit = 50): Promise<WorkflowFailure[]> => {
    const res = await api.get<WorkflowFailure[]>('/workflow-failures', { params: { limit } })
    return res.data
  },
  ...
  createFixPr: async (id: string): Promise<WorkflowFailureFixPRResult> => {
    const res = await api.post<WorkflowFailureFixPRResult>(`/workflow-failures/${id}/create-fix-pr`)
    return res.data
  },
}
...
export const repositoryService = {
  scan: async (repoFullName: string): Promise<RepositoryScanResult> => {
    const res = await api.post<RepositoryScanResult>('/repositories/scan', { repo_full_name: repoFullName })
    return res.data
  },
  createWorkflowPr: async (repoFullName: string): Promise<WorkflowPRResult> => {
    const res = await api.post<WorkflowPRResult>('/repositories/create-workflow-pr', { repo_full_name: repoFullName })
    return res.data
  },
}
...
export const cicdAssistantService = {
  predictFailure: async (logText: string): Promise<FailurePrediction> => {
    const res = await api.post<FailurePrediction>('/model/predict-failure', { log_text: logText })
    return res.data
  },
  generateWorkflow: async (files: string[]): Promise<GeneratedWorkflow> => {
    const res = await api.post<GeneratedWorkflow>('/cicd/generate-workflow', { files })
    return res.data
  },
}
